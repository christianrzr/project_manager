...
@section('styles')
<style>
    .board-page { max-width: 1440px; margin: 0 auto; }
    ...
    .board-intro h2 { color: var(--text-main); font-size: 1.7rem; letter-spacing: -.04em; }
    .board-intro p { color: var(--text-muted); font-size: .92rem; margin-top: 6px; }
    ...
    .column-title { align-items: center; color: #4a3630; display: flex; font-size: .9rem; font-weight: 760; gap: 8px; }
    .column-title svg { color: var(--column-color); height: 17px; width: 17px; }
    .column-count { background: #fff; border: 1px solid #e4d8d1; border-radius: 999px; color: #6e5a51; font-size: .75rem; font-variant-numeric: tabular-nums; font-weight: 750; padding: 3px 8px; }
...
<div class="board-page">
    <header class="board-intro"><div><h2>Keep work moving.</h2><p>Move tasks through each stage as your week takes shape.</p></div><button type="button" class="btn btn-primary" onclick="openCreateTaskModal()"><i data-lucide="plus" aria-hidden="true"></i>Add task</button></header>
